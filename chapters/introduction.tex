...
First, `fitness' is an intricate concept [\cite{Brommer2000} on 'the evolution of fitness in life-history theory], and it is unlikely that individuals can judge the full fitness implications of their movement decisions. 
...
Seemingly unimportant details of the movement process can result in quite different distributions in space, with different evolutionary implications \parencite{Houston & Lang (1998) and Netz et al. (2022)}. 
...
We propose models that \textit{work forwards from plausible mechanisms to potential emergent outcomes}.
...
For simplicity, as seen in the \textit{Case Study}, some ecological and evolutionary aspects will have to be set aside.
...
We conceive of individual movement across the landscape to take the form of sequential step-selection (which we call `movement decisions').
...
In our models, we assume individuals have a constant instantaneous speed, which means that all steps have the same distance.
...
We have presented a perspective on the need, usefulness, and implementation of individual-centric models that explicitly include animal movements as a key component.
These models are built from the ground up, with plausible individual behaviours as their fundamental processes.
This allows ecological phenomena --- including within-population individual variation, competition dynamics, and population social and spatial structure, to arise from feedbacks between the ecology and evolution of simple individual-level decision-making.
This class of models employs methods from artificial intelligence such as genetic algorithms, but rather than seeking to optimise for a specific problem, strives for conceptual insights into animal (movement) ecology and evolution.
We stress that this is how such models should be considered: as tools that enable the broad exploration of hypothetical scenarios, some of which we lay out below.
We further caution against expecting eco-evolutionary dynamics known from analytical models; for instance, while steady-state eco-evolutionary equilibria may emerge in some models \parencite[e.g.][]{gupte2021a,gupte2022c,getz2015,getz2016}, it is unrealistic to expect such dynamics from all models \parencite[see e.g.][]{netz2021}.
We encourage readers to adapt these scenarios to their own needs, and provide the simulation code for the case study as a starting point.

\subsection{Scenarios suited to investigation using our approach}

We focus on three broad yet relatively distinct classes of scenarios that are amenable to investigation using our mechanistic, eco-evolutionary modelling approach.
...
\paragraph{Changes in the environmental regime}

A key concern currently is knowing how the climate crisis is likely to affect animal spatial ecology, and we argue it is also important to know whether animal populations' evolutionary dynamics are likely to play a large role \parencite[e.g.][]{botero2015}.
For example, climate change is likely to induce greater variability in environmental conditions, thereby altering the spatial structure of resource landscapes (e.g. a transition from patchy to homogeneous resource landscapes [CITE]).
When resources are more homogenously distributed, direct resource cues ($F$ in our \textit{Case Study}) are likely to be more widespread, potentially reducing the importance of social information (conspecific presence), which could indicate a resource cluster [CITE].
...
This scenario could be studied by adapting our \textit{Case Study} to run for some thousands of generations (say, 3,000), then altering the spatial structure, and running the model for a few thousand generations more (e.g. initially having 30 patches of 15 items each, and then 90 patches, of 5 items each).
...
\paragraph{Joint evolution of movement and behavioural strategies}
...
In this context, it is not clear how prey type preferences would evolve, but movement and foraging strategies could potentially be correlated, making it an ideal case for exploration with the class of models we advocate.
This scenario could be explored by extending our \textit{Case Study} to have two overlapping prey type distributions, say $A$ and $B$, and allowing individuals to sense and have a preference for these different prey types ($s_A, s_B$, instead of $s_F$).
...
\paragraph{Introduction of new eco-evolutionary dynamics}
...
The class of mechanistic movement models we advocate could help explore whether current statistical tools can reliably detect individual differences in movement decision-making mechanisms.
...
\subsection{Developments and challenges in linking conceptual models with empirical data}

Our class of conceptual models aim to provide broad frameworks for the interpretation of current and future patterns observed in animal tracking data.
...
These could constitute simple initial tests of model predictions for the example scenarios we outlined earlier.
...
Within study systems, a key challenge for researchers seeking to estimate potential micro-evolutionary trajectories is likely to be estimating the fitness consequences of movement.
One approach could be to study movement in terms of energy expenditure, which can be reliably estimated from linked tri-axial accelerometry and tracking data for some species \parencite{stothart2016,shepard2013,gallagher2017}.
Linking individuals' energy expenditure with their survival and likely breeding success \parencite{picardi2020,schreven2021} could help parse the relationship between movement and these important components of fitness.
Furthermore, the analysis of tri-axial accelerometry data to detect feeding behaviour \parencite{nathan2012,chakravarty2020,williams2014,bryce2017} --- combined with remote sensing to detect forage quality and abundance, or tracking data to estimate the distributions of potential prey --- could be used to approximate individual energetic intake.
Mapping the landscape of energy expenditure, as well as landscapes of potential energetic gain, animals' fine-scale movement decisions could be linked directly to their energetic payoffs using `energy-based step selection analysis' \parencite{klappstein2022}.
Further developments of this approach could allow initial investigations of the evolutionary consequences of movement strategies.
