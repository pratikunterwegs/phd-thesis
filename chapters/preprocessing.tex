...
\begin{refsection}
    ...
    % \begin{figure}
    %     \centering
    %     \includegraphics[width=0.99\textwidth]{figures/fig_02.png}
    %     \caption{
    ...
    \begin{lstlisting}[float,floatplacement=h!,language=R, style=customR, caption = {
        ...
        The arguments \texttt{x} and \texttt{y} determine the X and Y coordinate columns, \texttt{x\_range} and \texttt{y\_range} are the filter bounds in a coordinate reference system in metres, and the data can be filtered by an \texttt{sf-(MULTI)POLYGON}, which can be passed using the \texttt{sf\_polygon} argument. 
    ...
    %         (1) Normally distributed small-scale error on each position, (2) large-scale error added to 0.5\% of positions, (3) 10\% of positions removed to simulate missing data, and (4) 300 consecutive positions displaced to simulate a gross distortion affecting a continuous subset of the track.
    ...
    %         This is because our method was designed to be robust to missing data from empirical tracks; the spatial and temporal limits of splitting and lumping can be controlled using the arguments passed to \texttt{atl\_res\_patch}, and can be adjusted to fit the study system.
