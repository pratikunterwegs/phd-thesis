...
% $$
% \text{nSpeciesOfInterest} \sim \textit{duration} + \text{sqrt(duration)} + \text{landcover} + \text{sqrt(time\_of\_day)} + \text{I((sqrt(time\_of\_day))^2)} + log(julian\_date) + I((log(julian\_date)^2)) + (1 | observer) 
% + (0 + duration | observer), family = \text{poisson})
% $$
